...
class WebGenerator:
    ...
    def generate_index(self, reports: list):
        ...
        ticker_html = ""
        if watchlist:
            ticker_html = '<div class="stock-ticker">'
            for s in watchlist:
                ticker_html += f"<span>{s}</span>"
            ticker_html += "</div>"

        cards = ""
        type_labels = {"pre_market": ("盘前必读", "type-pre"), "intraday": ("盘中快讯", "type-intra"), "post_market": ("盘后复盘", "type-post")}
        for report in reports:
            rtype = report.get("type", "")
            label, badge_class = type_labels.get(rtype, (rtype, ""))
            cards += f"""
<div class="report-card">
<a href="{report['html_path']}">
<span class="type-badge {badge_class}">{label}</span>
<h2>{report['title']}</h2>
<div class="meta">{report.get('created_at', '')}</div>
</a>
</div>"""
...
.type-badge {{ display: inline-block; padding: 2px 10px; border-radius: 10px; font-size: 12px; color: white; margin-bottom: 8px; }}
.type-pre {{ background: #e17055; }}
.type-intra {{ background: #00b894; }}
...
<p>本系统不构成投资建议，投资有风险，入市需谨慎</p>
    ...
    def _build_html(self, report_type: str, title: str, content: str, news: list = None) -> str:
        ...
        badge_class = {"pre_market": "type-pre", "intraday": "type-intra", "post_market": "type-post"}
        now = datetime.now().strftime("%Y-%m-%d %H:%M")
        watchlist_str = ", ".join(Config.STOCK_WATCHLIST) if Config.STOCK_WATCHLIST else "未设置"

        news_list = ""
        if news:
            news_list = '<div class="news-list">'
            for item in news[:50]:
                news_list += f"""
<div class="news-item">
...
<div class="news-title"><a href="{item.get('url', '#')}" target="_blank">{item['title']}</a></div>
...
.{badge_class.get(report_type, '')} {{ }}
.container {{ max-width: 800px; margin: 0 auto; padding: 20px; }}
.content {{ background: white; border-radius: 12px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }}
.content h2, .content h3 {{ margin: 24px 0 12px; padding-bottom: 8px; border-bottom: 2px solid #0984e3; }}
...
<div class="header">
<h1>{title}</h1>
<div class="meta">{now} | 自选股: {watchlist_str}</div>
</div>
<div class="container">
